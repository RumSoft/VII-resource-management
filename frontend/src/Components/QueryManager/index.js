...
class QueryManager extends Component {
  ...
  columnConfiguration = {
    ...
    resources: [
      ...
      },
      {
        ...
        selector: (x) =>
          x.attributes.length ? (
            x.attributes.map((y) => (
              <Label key={y.id} style={{ backgroundColor: y.color }}>
                {y.name}
              </Label>
            ))
          ) : (
      ...
      },
      {
        name: "Pokój",
        selector: (x) => (
          <Label style={{ backgroundColor: x.room && x.room.color }}>
            {" "}
            {`${x.room?.name || "brak pokoju"}`}
          </Label>
        ),
      },
    ],
    traderequests: [
      ...
      },
      {
        name: "ID zasobu",
        selector: (x) =>
          x.resource.id.substring(0, x.resource.id.indexOf("-")) + "...",
      },
      {
        name: "Nazwa",
      ...
      },
      {
        name: "Atrybuty",
        selector: (x) =>
          x.resource.attributes.length ? (
            x.resource.attributes.map((y) => (
              <Label key={y.id} style={{ backgroundColor: y.color }}>
                {y.name}
              </Label>
            ))
      ...
      },
      {
        ...
        selector: (x) => (
          <Label style={{ backgroundColor: x.room && x.room.color }}>
            {" "}
            {`${x.resource.room?.name || "brak pokoju"}`}
          </Label>
        ),
      },
    ],
        ...
        <Card.Content>
          <ContentLoader viewBox="0 0 100 20">
            {Array(7)
              .fill()
              .map((_, i) => (
                <rect key={i} x="1" y={i * 2 + 2} width="100" height="1" />
              ))}
          </ContentLoader>
        </Card.Content>
      </Card>
      ...
      <Table>
        <Table.Header>
          <Table.Row>
            {columns &&
              columns.map((x, i) => (
                <Table.HeaderCell key={i}>{x.name}</Table.HeaderCell>
              ))}
          </Table.Row>
        </Table.Header>
        <Table.Body>
          {entityList.map((x, i) => (
            <Table.Row key={i}>
              {columns.map((y, j) => (
                <Table.Cell key={j}>{y.selector(x)} </Table.Cell>
              ))}
            </Table.Row>
          ))}
        </Table.Body>
  ...
  changeSearchParams(obj) {
    this.setState(
      { ...obj, entityList: null, name: "", parentName: "" },
      () => {
        this.handleSubmit();
      }
    );
  ...
  render() {
    ...
    return (
      <>
        <Redirect from="/query" to="/query?type=rooms" />
        ...
        {entityList && !!entityList.length && (
          <PdfRenderer header={header.name} content={content} />
        )}
